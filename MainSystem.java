...
public class MainSystem {
    //Hero hero = new Hero();
    //Enemy superSon = new SuperSon("손오공", 1500, 150, 4, 50, 2000);
    // Enemy yasuo = new Yasuo();
    // Enemy reaper = new Reaper();
//    Enemy son = new Son("손오공", 1000, 100, 3, 30, 1000);
   ...
   /* public void characterChoice() {
        ...
        int num = sc.nextInt();
        if (num == 1) {
            hero = new Striker(name, 1, 0, 100, 91000, 1000, 1120, 1000);
        } else if (num == 2) {
    ...
    public void sparring() {
      /*  System.out.println("체육관 선택");
        ...
        if (number == 1) {
            ...
            Thread vsYasuoThread = new Thread(new VsYasuoThread());
            vsYasuoThread.start();
            while (true) {
                hero.공격하기(yasuo);
                if (yasuo.getHp() > 0) {
            ...
            Thread vsReaperThread = new Thread(new VsReaperThread());
            vsReaperThread.start();
            // fight-reaper
            while (true) {
                hero.공격하기(reaper);
                if (reaper.getHp() > 0) {
        ...
        // vs 손오공
            ...
            Thread vsSonThread = new Thread(new VsSonThread());
            vsSonThread.start();
            //Thread.interrupted(); //삭제
            ...
            Enemy superSon = new SuperSon("손오공", 1500, 150, 4, 50, 2000);
            //Thread vsBossAttack = new Thread(new VsBossAttack());
            VsBossAutoAttack vsBossAutoAttack = new VsBossAutoAttack();
            Thread th = new Thread(vsBossAutoAttack);
            //vsBossAttack.start();
            th.start();
            ...
            while (true) {
                ...
                if (superSon.getHp() > 0) {
                    ...
                    th.interrupt();
                    System.out.println(superSon.getName() + "!이(가) 쓰러졌습니다."+superSon.getHp());
   ...
   /* class VsBossAttack implements Runnable {
...
*/
    ...
    /*public static class VsYasuoThread implements Runnable {
        @Override
        public void run() {
            while (yasuo.getHp() > 0) {
                System.out.println("\n" + yasuo.getName() + "의 공격");
   ...
   /* public static class VsSonThread implements Runnable {
        @Override
        public void run() {
...
/*
    public class VsHero implements Runnable {
